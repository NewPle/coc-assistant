...
const Roll: React.VFC = () => {
  ...
  function counterRoll() {
    let diceResultP = d100(passives.additionalDice.value);
    if (passives.difficulty.type === "Hard") {
      diceResultP *= 2;
    } else if (passives.difficulty.type === "Extream") {
      diceResultP *= 5;
    }
    diceResultP += actives.additionalValue;

    let diceResultA = d100(actives.additionalDice.value);
    if (actives.difficulty.type === "Hard") {
      diceResultA *= 2;
    } else if (actives.difficulty.type === "Extream") {
      diceResultA *= 5;
    }
    diceResultA += actives.additionalValue;
  ...
  const CounterRoll = () => {
    return (
      <div>
        <IonList>
          <IonListHeader color={activeColor}>能動側の値を選択</IonListHeader>
          {sheets &&
            sheets.map((sheet) => {
              return (
                <SheetThumnail
                  ...
                  key={sheet.sheetId}
                  onSheetClick={() =>
                    setActives((prev) => ({ ...prev, sheetId: sheet.sheetId }))
                  }
                />
              );
            })}
        ...
        <IonList>
          <IonListHeader>追加のダイス</IonListHeader>
          <IonRadioGroup
            value={actives.additionalDice}
            onIonChange={(event) =>
              setActives((prev) => ({
                ...prev,
                additionalDice: event.detail.value,
              }))
            }
          >
            {ADDITIONAL_DICES.map((additionalDice, index) => {
        ...
        <IonList>
          ...
          <IonItem lines="full">
            <IonRange
              value={actives.additionalValue}
              onIonChange={(event) =>
                setActives((prev) => ({
                  ...prev,
                  additionalValue: Number(event.detail.value),
                }))
              }
              color={
                actives.additionalValue === 0
                  ? "dark"
                  : actives.additionalValue < 0
                  ? "danger"
                  : "primary"
        ...
        <IonList>
          <IonListHeader color={passiveColor}>受動側の値を選択</IonListHeader>
          {sheets &&
            sheets.map((sheet) => {
              return (
                <SheetThumnail
                  ...
                  key={sheet.sheetId}
                  onSheetClick={() =>
                    setPassives((prev) => ({ ...prev, sheetId: sheet.sheetId }))
                  }
                />
              );
            })}
        ...
        <IonList>
          ...
          <IonItem lines="full">
            <IonRange
              value={passives.additionalValue}
              onIonChange={(event) =>
                setPassives((prev) => ({
                  ...prev,
                  additionalValue: Number(event.detail.value),
  ...
  return (
    <IonPage>
      ...
      <IonContent>
        ...
        <IonModal
          isOpen={!!actives.sheetId}
          onDidDismiss={() => setActives((prev) => ({ ...prev, sheetId: "" }))}
        >
          <IonHeader>
            <IonToolbar>
              <IonButtons slot="end">
              ...
              <IonCard>
                <IonCardHeader>
                  <IonCardTitle>
                    {
                      sheets.filter(
                        (sheet) => sheet.sheetId === actives.sheetId
                      )[0].characterName
                    }
                  </IonCardTitle>
                </IonCardHeader>
                <IonCardContent>
                  ...
                  <IonList className="ion-padding-horizontal">
                    {sheets
                      .filter((sheet) => sheet.sheetId === actives.sheetId)[0]
                      .investigatorSkills.map((investigatorSkill, index) => {
                        return (
                          <IonChip
                            key={index}
        ...
        <IonModal
          isOpen={!!passives.sheetId}
          onDidDismiss={() => setPassives((prev) => ({ ...prev, sheetId: "" }))}
        >
          <IonHeader>
            <IonToolbar>
              <IonButtons slot="end">
              ...
              <IonCard>
                <IonCardHeader>
                  <IonCardTitle>
                    {
                      sheets.filter(
                        (sheet) => sheet.sheetId === passives.sheetId
                      )[0].characterName
                    }
                  </IonCardTitle>
                </IonCardHeader>
                <IonCardContent>
                  ...
                  <IonList className="ion-padding-horizontal">
                    {sheets
                      .filter((sheet) => sheet.sheetId === passives.sheetId)[0]
                      .investigatorSkills.map((investigatorSkill, index) => {
                        return (
                          <IonChip
                            key={index}
        ...
        {isCounterRoll ? <CounterRoll /> : <NormalRoll />}
      </IonContent>
    </IonPage>
  );
};
